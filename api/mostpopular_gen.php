...
//	"VALUES ($user_id, $family_id, ' ', '$transaction_id', ' ', $price, ' ', 'mostpopular', 'wire', false, '".date('Y-m-d' )."')";
//    $con->query($payment_sql);
	$payment = Payment::create([
			'user_id'=>$user_id,'payment_id'=>$transaction_id,'value'=>$price,'payment_type'=>'wire', 'confirmed'=>false,
			'date' => date('Y-m-d' ),'purpose'=>'mostpopular','family_id'=>$family_id
	]);
//    $con->query("UPDATE family SET mostpopular=2 where id={$family_id}");
	...
	$_SESSION['upgrade_successful'] = trans("mostpopular_wire");
	echo json_encode([
			'status' => true,
			'done' => true
	]);
	...
	$_SESSION['waiting_for_mostpopular'] = $user_id;
	...
	curl_setopt($ch, CURLOPT_POSTFIELDS,"apiOperation=CREATE_CHECKOUT_SESSION&interaction.operation=PURCHASE&apiPassword=c64c09161e4fba0ad70b46b77b5bc4e2&interaction.returnUrl=".$siteUrl.$RELATIVE_PATH."/mostpopular_response.php&apiUsername=merchant.E15701950&merchant=E15701950&order.id=$rand&order.currency=USD&order.amount=$price");
