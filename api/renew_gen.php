...
//			$con->query("UPDATE family SET mostpopular=0 where id={$family_id}");
		...
		$_SESSION['upgrade_successful'] = trans("renew_bank_transfer_success");
		echo json_encode([
				'status' => true,
				'done' => true
		]);
		...
		$_SESSION['waiting_for_renew_payment'] = $user_id;
		...
		curl_setopt($ch, CURLOPT_POSTFIELDS,"apiOperation=CREATE_CHECKOUT_SESSION&interaction.operation=PURCHASE&apiPassword=c64c09161e4fba0ad70b46b77b5bc4e2&interaction.returnUrl=".$siteUrl.$RELATIVE_PATH."/renew_plan_response.php&apiUsername=merchant.E15701950&merchant=E15701950&order.id=$rand&order.currency=USD&order.amount=$total_price");
	...
	echo json_encode([
			'status' => false,
			'error' => 1,
			'message' => trans('wrong_renew_plan'),
	]);
	die();
}
